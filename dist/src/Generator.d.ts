import { CaseBuilder } from './CaseBuilder';
export declare const Is: (value?: string, operation?: any) => CaseBuilder;
export declare const Case: (condition?: boolean, operation?: any) => CaseBuilder;
export declare const In: (...value: any[]) => CaseBuilder;
export declare const NotIn: (...value: any[]) => CaseBuilder;
export declare const InRange: (start: number, end: number, operation?: any) => CaseBuilder;
export declare const NotInRange: (start: number, end: number, operation?: any) => CaseBuilder;
export declare const Match: (regexp?: boolean, operation?: null) => CaseBuilder;
export declare const NotMatch: (regexp?: boolean, operation?: null) => CaseBuilder;
export declare const Else: (operation?: null) => CaseBuilder;
export declare const when: (...args: any) => any;
export declare const When: (...args: any) => any;
